...
function loadExamples() {
  if (!fs.existsSync(EXAMPLES_PATH)) {
    throw new Error(`Arquivo de exemplos mobile nao encontrado: ${EXAMPLES_PATH}`);
  }
...
function toPatient(porte = "pequeno") {
  if (porte === "grande") {
    return { species: "Mamifero", breed: "Bovino", ownerName: "Fazenda Campo" };
  }
  return { species: "Mamifero", breed: "Canino", ownerName: "Tutor Mobile" };
}
...
async function main() {
  ...
  if (!examples.length) {
    console.log("Nenhum exemplo mobile encontrado para avaliar.");
    return;
  }
  ...
  console.log("=== Avaliacao Chat Assist (Campo/Mobile) ===");
  console.table(rows);
...
main().catch((error) => {
  console.error("Falha ao avaliar chat assist mobile:", error);
  process.exit(1);
});
