...
async function main() {
  ...
  if (!examples.length) {
    logger.info('Nenhum exemplo mobile encontrado para avaliar.');
    return;
  }
  ...
  logger.info('=== Avaliacao Chat Assist (Campo/Mobile); ===');
  console.table(rows);
...
main().catch((error) => {
  logger.error('Falha ao avaliar chat assist mobile:', error);
  process.exit(1);
});
