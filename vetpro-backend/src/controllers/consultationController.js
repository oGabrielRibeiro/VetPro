...
function parseChatHistoryFromNotes(notes = '') {
  ...
  const raw = text.slice(start + CHAT_MARK_START.length, end).trim();
  if (!raw) return [];
...
async function create(req, res) {
  try {
    ...
    try {
      ...
      const sourceText = normalizeTimestampedTranscript(
        notesTranscript || data?.transcript || '',
      );
      const parsed = {
        chiefComplaint: String(data?.chiefComplaint || '').trim(),
        anamnesis: String(data?.anamnesis || '').trim(),
        physicalExam: String(
          data?.physicalExam || data?.clinicalAssessment || '',
        ).trim(),
      ...
      const populatedCount = Object.values(parsed).filter((value) =>
        String(value || '').trim(),
      ).length;
...
async function chatAssist(req, res) {
  try {
    ...
    const transcript = String(req.body?.transcript || '').trim();
    const messagesRaw = Array.isArray(req.body?.messages)
      ? req.body.messages
      : [];
    const segmentsRaw = Array.isArray(req.body?.segments)
      ? req.body.segments
      : [];
    ...
    const recordProfile =
      recordProfileRaw && typeof recordProfileRaw === 'object'
        ? {
            porte: String(recordProfileRaw.porte || '')
              .trim()
              .toLowerCase(),
