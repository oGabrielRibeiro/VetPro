...
function renderPrescription(doc, data) {
  ...
  doc
    ...
    .fontSize(18)
    .fillColor("#ffffff")
    .text(data.vet?.name || data.clinic?.name || "Dr. Vet", headerX + 14, headerY + 16, {
      width: headerW - 90,
      align: "left"
    });
  doc
  ...
  doc
    ...
    .fontSize(10)
    .fillColor(muted)
    .text(`Receita No ${String(data.consultation.recipeNumber || 0).padStart(6, "0")}/${data.consultation.recipeYear || new Date().getFullYear()}`, bodyLeft, y, {
      width: bodyWidth,
      align: "right"
    });
  y += 12;
  ...
  const fields = [
    ...
    { label: "Peso", value: `${data.consultation.weight || "Nao informado"} kg` }
  ];
  ...
  // Diagnosis line
  doc.font("Helvetica").fontSize(7).fillColor(muted).text("Diagnostico:", bodyLeft, y, { width: bodyWidth });
  doc
    .lineWidth(0.8)
    .strokeColor("#9fb1c1")
  ...
  doc
    ...
    .fontSize(8)
    .fillColor(muted)
    .text((data.clinic?.name || "CLINIC NAME").toUpperCase(), bodyLeft, sheetY + sheetH - 14);
  doc
    .font("Helvetica")
    .fontSize(7)
    .fillColor(muted)
    .text(data.clinic?.address || "-", bodyLeft + 130, sheetY + sheetH - 14, { width: 190 });
  doc
    .font("Helvetica")
    .fontSize(7)
    .fillColor(muted)
