...
function safeFieldSentence(value = '', maxLen = 140) {
  return truncateText(
    trimByPlanningTail(trimBySentenceBoundary(value)),
    maxLen,
  );
}
...
function looksLikeNoisyConversation(value = '') {
  ...
  const noiseHits = noiseTokens.filter((token) =>
...
function normalizeSpecificValueByKey(key, value = '') {
  ...
  if (key === 'rationBrand') {
    ...
    const looksLikeRation =
      /\b(racao|ração|premium|seca|umida|úmida|marca|dieta)\b/.test(
        normalizedRation,
      );
  ...
  if (key === 'allergyHistory') {
    ...
    const looksLikeAllergy = /\b(alerg|atopi|sazonal|reacao|prurido)\b/.test(
      normalizedAllergy,
    );
...
function inferSpecificFieldValueFromContext(
  ...
  if (key === 'behavior') {
    const fromComplaint = extractClinicalComplaintFromConversation(
      chiefComplaint || text,
      180,
    );
  ...
  if (key === 'vaccinationStatus') {
    if (
      /\b(vacina|vacinacao|vacinas)\b/.test(normalized) &&
      /\bem dia\b/.test(normalized)
    )
      return 'Em dia';
    if (
      /\b(vacina|vacinacao|vacinas)\b/.test(normalized) &&
      /\b(atras|desatual|incomplet)\b/.test(normalized)
    ) {
      return 'Atrasada';
    }
  }
  ...
  if (key === 'waterIntakeSmall') {
    if (
      /\b(polidips|bebe muita agua|ingestao aumentada|aumentou a agua)\b/.test(
        normalized,
      )
    )
      return 'Aumentada';
    if (
      /\b(nao bebe|recusa agua|ingestao reduzida|bebe pouco|desidrat)\b/.test(
        normalized,
      )
    )
      return 'Diminuida';
  ...
  if (key === 'diet') {
    return normalizeSpecificValueByKey(
      key,
      extractClinicalSentenceByTerms(text, [
        'racao',
        'ração',
        'dieta',
        'alimentacao',
  ...
  if (key === 'vaccinationProtocol') {
    const protocolMatch = text.match(
      /\b(v8|v10|v11|antirrabic[ao]?|giardia|kc|pneumodog)\b/gi,
    );
    if (protocolMatch?.length) {
      return normalizeSpecificValueByKey(
        key,
        Array.from(
          new Set(protocolMatch.map((item) => item.toUpperCase())),
        ).join(', '),
      );
    }
    const protocolSentence = extractClinicalSentenceByTerms(text, [
      'protocolo vacinal',
      'vacina',
      'vacinacao',
    ]);
  ...
  if (key === 'productionSystem') {
    if (/\b(lactacao|leite|producao leiteira)\b/.test(normalized))
      return 'Leite';
  ...
  if (key === 'batch') {
    const explicitBatch = extractFirstMatch(
      text,
      /\b(?:lote|grupo|piquete)\s*(?:n[ºo]\s*)?([a-z0-9\-]+)/i,
    );
    if (explicitBatch)
      return normalizeSpecificValueByKey(key, `Lote ${explicitBatch}`);
    return normalizeSpecificValueByKey(
      key,
      extractClinicalSentenceByTerms(text, [
        'lote',
  ...
  if (key === 'animalId') {
    const explicitId = extractFirstMatch(
      text,
      /\b(?:vaca|boi|bezerro|equino|animal|brinco|id)\s*[:#]?\s*([a-z0-9\-]{2,})/i,
    );
    return normalizeSpecificValueByKey(key, explicitId);
  }

  if (key === 'bodyConditionScore') {
    const bcs = extractFirstMatch(
      text,
      /\b(?:ecc|escore corporal)\s*[:=]?\s*([1-5](?:[.,]\d)?)/i,
    );
    return normalizeSpecificValueByKey(key, bcs);
  }

  if (key === 'daysInMilk') {
    const del = extractFirstMatch(
      text,
      /\b(?:del|dias?\s+em\s+lactacao|dias?\s+lactacao)\s*[:=]?\s*(\d{1,4})/i,
    );
    return normalizeSpecificValueByKey(key, del);
  }

  if (key === 'parity') {
    const parity = extractFirstMatch(
      text,
      /\b(?:paridade|partos?)\s*[:=]?\s*(\d{1,2})/i,
    );
