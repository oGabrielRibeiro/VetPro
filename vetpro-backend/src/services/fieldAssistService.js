...
const HEURISTIC_MEMORY_PATH = path.join(
  __dirname,
  '..',
  'ai',
  'heuristicMemory.json',
);
...
function detectSentenceCategory(sentence = '') {
  ...
  if (!best || best.score <= 0)
    return { category: 'irrelevante', confidence: 0.2 };

  const confidence = Math.min(
    0.95,
    Number((0.35 + best.score * 0.15).toFixed(2)),
  );
...
function applySentenceToState(state, classifiedSentence) {
  ...
  const concepts = Array.isArray(classifiedSentence?.concepts)
    ? classifiedSentence.concepts
    : [];
...
function classifyAndExtractIncremental(segments = [], transcript = '') {
  ...
  const sourceSentences = safeSegments.length
    ...
    : splitSentencesWithMetadata(transcript, {
        stamp: '00:00',
        speaker: 'Tutor',
      });
...
function evaluateClinicalRules(transcript = '', extractionState = {}) {
  ...
  const hasShockPattern =
    (/tpc\s*([3-9]|[1-9]\d)/.test(normalized) ||
      normalized.includes('tpc 3')) &&
    (normalized.includes('palida') ||
      normalized.includes('pálida') ||
      normalized.includes('icter'));
  if (hasShockPattern) {
    alerts.push({
      id: 'choque_circulatorio',
      severity: 'alta',
      message:
        'Padrao compativel com hipoperfusao/choque circulatorio; reavaliar urgencia imediatamente.',
    });
  }

  const hasColicPattern =
    (normalized.includes('rolar') ||
      normalized.includes('flanco') ||
      normalized.includes('dor abdominal')) &&
    (normalized.includes('colica') ||
      normalized.includes('hipomotilidade') ||
      normalized.includes('inquiet'));
  if (hasColicPattern) {
    alerts.push({
      id: 'suspeita_colica',
      severity: 'alta',
      message:
        'Sinais sugerem colica; considerar monitorizacao continua e analgesia protocolada.',
    });
  }

  const hasDehydrationPattern =
    (normalized.includes('agua diminu') ||
      normalized.includes('nao bebe') ||
      normalized.includes('não bebe')) &&
    (normalized.includes('urina concentrada') ||
      normalized.includes('fezes ressec'));
  if (hasDehydrationPattern) {
    alerts.push({
      id: 'desidratacao',
      severity: 'media',
      message:
        'Sinais de desidratacao; considerar fluidoterapia e monitorar perfusao.',
    });
  }
  ...
  const coherence = clampValue(
    alerts.length * 0.2 + evidenceCount * 0.05,
    0,
    1,
  );
...
function buildParsedFromExtractionState(
  state = {},
  fallbackText = '',
  roleData = {},
) {
  ...
  const fullContent = String(
    roleData?.fullContent || fallbackText || '',
  ).trim();

  const chiefComplaintDraft = dedupeAndJoin([
    ...(state.queixa || []),
    ...(state.sintoma || []),
  ]).slice(0, 260);
  ...
  return {
    chiefComplaint:
      extractByKeywords(tutorContent || fullContent, [
        'queixa',
        'motivo da consulta',
        'motivo',
      ]) ||
...
function classifySentencesHeuristic(text = '') {
  ...
  for (const sentence of sentences) {
    ...
    if (isMatch(normalized, medicationTokens))
      buckets.medications.push(sentence);
  }
...
function assessRoleReliability(
  segments = [],
  tutorContent = '',
  medicoContent = '',
) {
  ...
  const tutorTurns = safeSegments.filter(
    (segment) => segment?.speaker === 'Tutor',
  ).length;
  const medicoTurns = safeSegments.filter(
    (segment) => segment?.speaker === 'Medico',
  ).length;
  ...
  const signalBalance =
    1 -
    Math.abs(tutorSignals - medicoSignals) /
      Math.max(1, tutorSignals + medicoSignals);
  const coverage = Math.min(1, (tutorSignals + medicoSignals) / 8);

  const score = Number(
    (turnBalance * 0.35 + signalBalance * 0.25 + coverage * 0.4).toFixed(2),
  );
...
function detectSpeakerFromSentence(sentence = '', lastSpeaker = 'Tutor') {
  ...
  const symptomContext =
    /\b(nao ta bem|não tá bem|sem apetite|vomit|diarre|febre|dor|apat|prostr|mucosa|coce)\b/.test(
      normalized,
    );
  const vetQuestionContext =
    /\b(desde quando|ha quanto|há quanto|me explica|me conte|me conta|pode me dizer)\b/.test(
      normalized,
    );
  const vetActionContext =
    /\b(vamos|no exame|ao exame|suspeita|diagnost|conduta|tratamento|prescrev|oriento|solicitei|pedi|coletar)\b/.test(
      normalized,
    );
...
function buildHeuristicTurnsFromTranscript(transcript = '') {
  ...
  for (const sentence of sentences) {
    ...
    if (existing && existing.speaker === speaker) {
      existing.text = `${existing.text}. ${sentence}`
        .replace(/\s+/g, ' ')
        .trim();
      continue;
    }
...
function writeHeuristicMemory(examples = []) {
  try {
    const payload = {
      examples: optimizeHeuristicMemory(
...
function scoreMemoryExample(example = {}) {
  ...
  const parsed =
    example?.parsed && typeof example.parsed === 'object' ? example.parsed : {};
  ...
  return Number(
    Math.min(1, coverageScore * 0.6 + lengthScore + richnessScore).toFixed(4),
  );
}
...
function normalizeMemoryExample(example = {}) {
  return {
    ...
    sourceText: String(example?.sourceText || '')
      .replace(/\s+/g, ' ')
      .trim(),
    parsed: {
...
function optimizeHeuristicMemory(examples = []) {
  ...
  for (const entry of scored) {
    const duplicateIndex = deduped.findIndex(
      (stored) =>
        jaccardSimilarity(entry.item.sourceText, stored.item.sourceText) >=
        0.92,
    );
    if (duplicateIndex < 0) {
      deduped.push(entry);
    ...
    if (
      ...
      (entry.score === existing.score && entryDate > existingDate)
    ) {
  ...
  const bestKeys = new Set(
    best.map((entry) => normalizeText(entry.item.sourceText)),
  );
...
function extractTranscriptFromNotes(notes = '') {
  ...
  const timed = source.match(
    /Transcricao com minutagem:\s*([\s\S]*?)(?:\n{2,}|$)/i,
  );
  if (timed?.[1]) return normalizeTimestampedTranscript(timed[1]);
  const plain = source.match(
    /Transcricao da conversa:\s*([\s\S]*?)(?:\n{2,}|$)/i,
  );
...
function applyMemoryToParsed(parsed = {}, sourceText = '') {
  ...
  const reference =
    best.item?.parsed && typeof best.item.parsed === 'object'
      ? best.item.parsed
      : {};
...
function parseClinicalFieldsFromSegments(segments = [], transcript = '') {
  ...
  const roleReliability = assessRoleReliability(
    safeSegments,
    tutorContent,
    medicoContent,
  );
  ...
  const tutorOrFull = shouldUseFullAsPrimary
    ? fullContent
    : tutorContent || fullContent;
  const medicoOrFull = shouldUseFullAsPrimary
    ? fullContent
    : medicoContent || fullContent;
  const { classified: classifiedSentences, state: extractionState } =
    classifyAndExtractIncremental(safeSegments, fullContent);
...
function buildParsedConfidence(parsed, context, provider) {
  return {
    chiefComplaint: scoreField(
      parsed.chiefComplaint,
      context.tutorContent,
      provider,
    ),
    anamnesis: scoreField(parsed.anamnesis, context.tutorContent, provider),
    physicalExam: scoreField(
      parsed.physicalExam,
      context.medicoContent,
      provider,
    ),
...
function scoreSpeakerRole(text = '') {
  ...
  if (
    /\b(dr|dra|doutor|doutora)\b/.test(normalized) &&
    /\b(ele|ela|nao|não)\b/.test(normalized)
  ) {
    tutor += 2;
  }
...
function buildCompositeConfidence(parsedConfidence = {}, pipeline = {}) {
  ...
  Object.keys(map).forEach((field) => {
    ...
    const evidenceCount = sources.reduce(
      (sum, bucket) => sum + (state?.[bucket]?.length || 0),
      0,
    );
    const coverage = sources.length
      ? sources.reduce(
          (sum, bucket) =>
            sum + Math.min(1, (state?.[bucket]?.length || 0) / 2),
          0,
        ) / sources.length
      : 0;
...
function shouldWriteDebugArtifacts() {
  const forced = String(process.env.FIELD_ASSIST_DEBUG || '')
    .trim()
    .toLowerCase();
...
function writeDebugArtifacts(payload = {}) {
  ...
  try {
    ...
    const files = [
      ...
      {
        suffix: 'frases_classificadas',
        data: payload.classifiedSentences || [],
      },
    ...
    for (const file of files) {
      const fullPath = path.join(
        DEBUG_DIR_PATH,
        `${stamp}-${file.suffix}.json`,
      );
      fs.writeFileSync(fullPath, JSON.stringify(file.data, null, 2), 'utf8');
    }
  } catch (error) {
    console.error(
      'Falha ao gravar logs de debug do modo campo:',
      error.message,
    );
  }
}
...
async function analyzeFieldConversation({
  ...
  const parsedConfidenceComposite = buildCompositeConfidence(
    parsedConfidence,
    pipeline,
  );
  ...
  const result = {
    ...
    pipeline: {
      ...
      semanticRules: pipeline?.semanticRules || {
        alerts: [],
        coherenceScore: 0,
      },
